...
pub fn init(parent: *Connection, id: u31, allocator: std.mem.Allocator) Stream {
    return Stream{ 
        .id = id,
        .parent = parent,
        .peer_window_size = parent.peer_settings.initial_window_size,
        .state = .idle,
...
pub fn sendHeaders(self: *Stream, headers: []hpack.HeaderField, eos: bool) !void {
    ...
    try hproc.write(
        ...
        @truncate(self.parent.peer_settings.max_header_list),
...
pub fn write(self: *Stream, buf: []const u8, padding: []const u8, eos: bool) !void {
    ...
    while (tosend.len > 0 and self.windowAvailable()) {
        ...
        try frames.Data.write(self.parent.stream.writer(),self.id, tosend[0..offset],"",
         if(offset == tosend.len) eos else false
        );
        tosend = tosend[offset..];
        //self.peer_window_size -= @intCast(offset);
...
pub fn writePending(self: *Stream) !void {
    ...
    while (self.pendingbuf.len > 0 and self.windowAvailable()) {
        ...
        try frames.Data.write(self.parent.stream.writer(),self.id, self.pendingbuf[0..offset],"",
         if(offset == self.pendingbuf.len) true else false
        );
