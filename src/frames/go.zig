...
pub const PayLoad = struct { last_id: u31, code: errors.Code, debug_info: ?[]const u8 = null };
...
pub fn write(stream: anytype, payload: PayLoad) !void {
    var p = Head{ .flags = .{}, .len = @truncate(8 + if (payload.debug_info) |info| info.len else 0), .streamid = 0, .ty = .goaway };
    try p.write(stream);
