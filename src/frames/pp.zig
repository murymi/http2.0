...
pub fn write(
    ...
    if (hpack.Field.getHeaderFieldLen(headers) <= max_header_list) {
        ...
        var header = frame.Head{ .flags = .{ .endheaders = true }, .len = @intCast(hfin.len), .streamid = id, .ty = .pushpromise };
        ...
        while (fitting.b.len > 0) {
            ...
            const hfin = self.builder.final();
            var h = frame.Head{ .flags = .{}, .len = @intCast(hfin.len), .streamid = id, .ty = if(first_round) .pushpromise else .continuation };
            try h.write(stream);
            if(first_round) try stream.writeInt(u32, promiseId);
            try stream.writeAll(hfin);
        ...
        try self.builder.addSlice(fitting.a);
        const hfin = self.builder.final();
        var finalhead = frame.Head{ .flags = .{ .endheaders = true }, .len = @intCast(hfin.len), .streamid = id, .ty = if(first_round) .pushpromise else .continuation };
...
pub fn read(instream: anytype, out: []u8, head: Head, id: *u32) !usize {
    var hd = head;
    var r: usize = 0;
    ...
    id.* = try instream.readInt(u32, .big) & -idmask;
