...
pub fn init(allocator: std.mem.Allocator, ctx: *t) Self {
    return Self{ .allocator = allocator, .buf = std.ArrayList(u8).init(allocator), .ctx = ctx };
}
...
test "plain" {
    ...
    defer ctx.deinit();
    var b = @This().init(malloc, &ctx);
    defer b.deinit();

    {
        defer b.clear();
        try b.add(.{ .name = "custom-key", .value = "custom-header" }, true, false);
        try std.testing.expectEqualSlices(u8, &.{ 0x40, 0x0a, 0x63, 0x75, 0x73, 0x74, 0x6f, 0x6d, 0x2d, 0x6b, 0x65, 0x79, 0x0d, 0x63, 0x75, 0x73, 0x74, 0x6f, 0x6d, 0x2d, 0x68, 0x65, 0x61, 0x64, 0x65, 0x72 }, b.buf.items);
    }
    {
        defer b.clear();
        try b.add(.{ .name = ":path", .value = "/sample/path" }, false, false);
        try std.testing.expectEqualSlices(u8, &.{ 0x04, 0x0c, 0x2f, 0x73, 0x61, 0x6d, 0x70, 0x6c, 0x65, 0x2f, 0x70, 0x61, 0x74, 0x68 }, b.buf.items);
    }
    {
        defer b.clear();
        try b.add(.{ .name = "password", .value = "secret" }, false, true);
        try std.testing.expectEqualSlices(u8, &.{ 0x10, 0x08, 0x70, 0x61, 0x73, 0x73, 0x77, 0x6f, 0x72, 0x64, 0x06, 0x73, 0x65, 0x63, 0x72, 0x65, 0x74 }, b.buf.items);
    }
    {
        defer b.clear();
        defer ctx.clear();
        try b.add(.{ .name = ":method", .value = "GET" }, false, false);
        try std.testing.expectEqualSlices(u8, &.{0x82}, b.buf.items);
    }
    {
        defer b.clear();
        //defer ctx.clear();
        ...
        try std.testing.expectEqualSlices(u8, &.{ 0x82, 0x86, 0x84, 0x41, 0x0f, 0x77, 0x77, 0x77, 0x2e, 0x65, 0x78, 0x61, 0x6d, 0x70, 0x6c, 0x65, 0x2e, 0x63, 0x6f, 0x6d }, b.buf.items);
    ...
    }
    {
        ...
        try std.testing.expectEqualSlices(u8, &.{ 0x82, 0x86, 0x84, 0xbe, 0x58, 0x08, 0x6e, 0x6f, 0x2d, 0x63, 0x61, 0x63, 0x68, 0x65 }, b.buf.items);
    ...
    }
    {
        ...
        try std.testing.expectEqualSlices(u8, &.{ 0x82, 0x87, 0x85, 0xbf, 0x40, 0x0a, 0x63, 0x75, 0x73, 0x74, 0x6f, 0x6d, 0x2d, 0x6b, 0x65, 0x79, 0x0c, 0x63, 0x75, 0x73, 0x74, 0x6f, 0x6d, 0x2d, 0x76, 0x61, 0x6c, 0x75, 0x65 }, b.buf.items);
...
test "compress" {
    ...
    defer b.deinit();
    {
        ...
        try std.testing.expectEqualSlices(u8, &.{ 0x82, 0x86, 0x84, 0x41, 0x8c, 0xf1, 0xe3, 0xc2, 0xe5, 0xf2, 0x3a, 0x6b, 0xa0, 0xab, 0x90, 0xf4, 0xff }, b.buf.items);
        try testing.expectEqual(57, ctx.dynamic_table.capacity);
    }
    {
        defer b.clear();
        ...
        try std.testing.expectEqualSlices(u8, &.{ 0x82, 0x86, 0x84, 0xbe, 0x58, 0x86, 0xa8, 0xeb, 0x10, 0x64, 0x9c, 0xbf }, b.buf.items);
        try testing.expectEqual(110, ctx.dynamic_table.capacity);
    }
    {
        defer b.clear();
        ...
        try std.testing.expectEqualSlices(u8, &.{ 0x82, 0x87, 0x85, 0xbf, 0x40, 0x88, 0x25, 0xa8, 0x49, 0xe9, 0x5b, 0xa9, 0x7d, 0x7f, 0x89, 0x25, 0xa8, 0x49, 0xe9, 0x5b, 0xb8, 0xe8, 0xb4, 0xbf }, b.buf.items);
        try testing.expectEqual(164, ctx.dynamic_table.capacity);
    }
}
