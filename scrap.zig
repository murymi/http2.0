...
pub fn maintt() !void {
    ...
    var pp = frames.PushPromise{
        .builder = hpack.Builder.init(&ctx, buf[0..]),
        .parser = hpack.Parser.init(&ctx, headerheap[0..])
    };

    var expected = [_]hpack.HeaderField{ 
...
pub fn main() !void {
    ...
    var stream = std.io.fixedBufferStream(streambuf[0..]);
        ...
        .{ .name = "", .value = "" },
         .{ .name = "", .value = "" },
          .{ .name = "three", .value = "Mon, 21 Oct 2013 20:13:21 GMT" },
           .{ .name = "four", .value = "https://www.example.com" } 
        };
